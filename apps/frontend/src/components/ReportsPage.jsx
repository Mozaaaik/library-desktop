...
export default function ReportsPage() {
    ...
    const fetchDateRange = async () => {
        ...
        try {
            // 1) hangi üyelerden çekeceğiz?
            const memberIds =
                selectedMemberId !== "all"
                    ? [selectedMemberId]
                    : members.map((m) => String(m.id ?? m.UyeID ?? m.MemberID));

            // 2) her üyenin loans'unu çek
            ...
            // 4) normalize (senin findLoans alanlarına göre gerekirse düzelt)
            // Beklenen alanlar: memberName, studentNo, bookName, category, loanDate, dueDate, returnDate
            rows = rows.map((x, idx) => {
                const row = {
                    id: x.id ?? x.LoanID ?? `${idx}-${Math.random()}`,
                    memberName: x.memberName ?? x.UyeAdi ?? x.AdSoyad ?? x.name ?? "-",
                    studentNo: x.studentNo ?? x.OgrenciNo ?? x.ogrenciNo ?? "-",
                    bookName: x.bookName ?? x.Baslik ?? x.title ?? "-",
                    category: x.category ?? x.KategoriAdi ?? x.kategori ?? "-",
                    loanDate: (x.loanDate ?? x.OduncTarihi ?? x.tarih ?? "").slice(0, 10),
                    dueDate: (x.dueDate ?? x.SonTeslimTarihi ?? x.due ?? "").slice(0, 10),
                    returnDate: x.returnDate ? String(x.returnDate).slice(0, 10) : null,
                };
            ...
            // 5) filtreler
            rows = rows.filter((r) => {
                if (startDate && r.loanDate && r.loanDate < startDate) return false;
                if (endDate && r.loanDate && r.loanDate > endDate) return false;
                if (selectedCategory !== "all" && r.category !== selectedCategory) return false;

                if (selectedStatus !== "all") {
                    if (selectedStatus === "returned" && r.status !== "returned") return false;
                    if (selectedStatus === "active" && (r.status !== "active" && r.status !== "overdue")) return false;
    ...
    const fetchMostBorrowed = async () => {
        ...
        try {
            const memberIds = members.map((m) => String(m.id ?? m.UyeID ?? m.MemberID));
            ...
            // tarih filtresi
            rows = rows.filter((r) => {
                if (borrowStartDate && r.loanDate && r.loanDate < borrowStartDate) return false;
                if (borrowEndDate && r.loanDate && r.loanDate > borrowEndDate) return false;
                if (borrowCategory !== "all" && r.category !== borrowCategory) return false;
                return true;
            });
    ...
    const exportPDF = () => {
        if (activeTab === "date-range" && dateLoaded) {
            ...
            exportPdfBlob("tarih_araligi_odunc.pdf", "Tarih Aralığına Göre Ödünç Raporu", cols, dateRows);
            return;
        }
        ...
        if (activeTab === "overdue" && overdueLoaded) {
            ...
            exportPdfBlob("geciken_kitaplar.pdf", "Geciken Kitaplar Raporu", cols, overdueRows);
            return;
        }
        ...
        if (activeTab === "most-borrowed" && mostLoaded) {
            ...
            exportPdfBlob("en_cok_odunc_alinan.pdf", "En Çok Ödünç Alınan Kitaplar", cols, mostRows);
            return;
        }
